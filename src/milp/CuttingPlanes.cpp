...
int selectBranchVarPseudoCost(const std::vector<double>&   sol,
    ...
    for (uint32_t id : intVarIds) {
        ...
        if (pc.upCount[id] > 0)
            dUp = (pc.upCost[id] / static_cast<double>(pc.upCount[id])) * (1.0 - fr);
        else
            dUp = std::min(fr, 1.0 - fr); // MostFractional fallback

        if (pc.downCount[id] > 0)
            dDown = (pc.downCost[id] / static_cast<double>(pc.downCount[id])) * fr;
        else
            dDown = std::min(fr, 1.0 - fr);
...
void updatePseudoCosts(PseudoCosts& pc,
    ...
    if (branchedUp) {
        pc.upCost[varId] += rate;
        ++pc.upCount[varId];
    } else {
        pc.downCost[varId] += rate;
        ++pc.downCount[varId];
    }
}
