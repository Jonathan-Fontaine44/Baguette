...
| `v0.6.4` | ✅ Done | **Multi-level MILP presolve** (levels 0–6): `BBOptions::presolveLevel` remplace `bool enablePresolve`. Niveau 1 = LP bound-tightening + arrondi entier + PR1 (comportement existant, défaut) ; niveau 2 = + propagation CP fixpoint au root avant le B&B ; niveau 3 = + weak probing (fixe chaque variable binaire, propage via niveau 2, intersecte les bornes résultantes) ; niveau 4 = + LP relaxation au root (détection d'infaisabilité LP) ; niveau 5 = + injection de lignes d'implication binaires découvertes par le probing (plafonnées par `maxImpliedRows`) ; niveau 6 = + strong probing (LP solve par fixation binaire, détecte les infaisabilités que la propagation ne peut pas). Chaque niveau implique tous les niveaux inférieurs. Nouveaux champs `BBOptions` : `probingMaxVars` (50), `maxImpliedRows` (100), `probingLPMethod` (`DualSimplexBV`). `MILPPresolveResult` étendu avec `varsProbed`, `varsProbedFixed`, `impliedRowsAdded`. Nouvelle struct `MILPPresolveOpts` pour l'API directe de `presolveMILPInPlace`. |
