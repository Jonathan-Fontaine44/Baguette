...
| `v0.5.1` | Planned | **Short-step feasible path-following IPM**: starts from a strictly interior-feasible point (x > 0, s > 0, Ax = b, Aᵀy + s = c). At each iteration solves the Newton system for the direction toward the central path (barrier parameter μ = xᵀs/n) and takes a fixed short step α = O(1/√n) to remain in the 2-norm neighborhood N₂(θ). Convergence in O(√n log(1/ε)) iterations — theoretically clean and simpler to implement than Mehrotra, at the cost of more iterations in practice. Foundation for v0.5.2. |
| `v0.5.2` | Planned | **Primal-dual infeasible IPM + Mehrotra predictor-corrector**: no feasibility requirement for the starting point — primal/dual residuals (rp = b − Ax, rd = c − Aᵀy − s) and the duality gap shrink jointly. Mehrotra predictor-corrector: an affine-scaling direction (μ = 0) predicts the step length αaff, then a centering-corrector direction with adaptive σ = (μaff/μ)³ keeps the iterate in the neighborhood. Typically 15–50 iterations in practice vs O(√n) theoretical; standard approach in industrial LP solvers. |
