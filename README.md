...
| `v0.2.3` | Planned | B&B warm-start via dual simplex: `solveDualFrom(Model, BasisRecord)` — initialise the child-node tableau directly from the parent's optimal `BasisRecord` (skipping Phase I), then run the dual simplex to restore primal feasibility after the branching constraint is added. Converges on average in O(1) pivots per node, as branching only violates a single constraint. |
| `v0.2.4` | Planned | Farkas certificate: when `solveDetailed` / `solveDualDetailed` returns `Infeasible`, populate a `FarkasRay` in `LPDetailedResult` — a vector `y` extracted from the blocking tableau row such that `A^T y ≥ 0` and `b^T y < 0`, providing a machine-verifiable proof of infeasibility. Used in B&B to generate infeasibility cuts and prune sibling nodes without re-solving. |
| `v0.2.5` | Planned | Sensitivity analysis: `SensitivityReport` populated by `sensitivityAnalysis(LPDetailedResult)` from the optimal tableau without re-solving. Reports per-constraint **RHS ranging** (interval `[b_i − δ⁻, b_i + δ⁺]` over which the current basis stays primal feasible, computed via ratio tests on `B⁻¹b`) and per-variable **objective ranging** (interval `[c_j − δ⁻, c_j + δ⁺]` over which the basis stays dual feasible, computed via ratio tests on reduced costs). Shadow prices and reduced costs are already available in `LPDetailedResult`; this adds the stability intervals needed for what-if analysis and B&B bound tightening. |
