...
| `v0.3.1` | Planned | Branch & Cut: Gomory fractional cuts from the final tableau, MIR cuts, cut pool with trail-based activation per node. Pseudo-cost branching. New: `CuttingPlanes.hpp/.cpp`. |
| `v0.3.2` | Planned | Node memory optimisation: replace full bounds snapshots (`O(n)` per node) with per-branch **delta records** (`{varId, oldLb, oldUb, newLb, newUb}`) and a trail-based restore. Reduces peak memory from `O(n × nodes)` to `O(depth_max)` in the common case. |
| `v0.3.3` | Planned | Hybrid node-selection strategy: **depth-first plunge** until the first integer-feasible solution is found, then automatic switch to **best-bound** to prove optimality. Extends `NodeSelection` with a `HybridPlunge` mode. Combines fast incumbent finding with tight bound convergence. |
