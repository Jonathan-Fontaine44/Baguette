...
| `v0.4.2` | Planned | B&C profiling: extend `MILPResult` with opt-in granular statistics controlled by a `BBOptions::collectStats` flag (default false, zero overhead when disabled). Planned fields: `nodesWithCuts` (nodes that generated ≥1 GMI cut), `nodesPrunedByBound` / `nodesPrunedByInfeasibility` (pruning breakdown), `warmStartFallbacks` (sibling nodes that fell back to cold start after a cut), `lpSolvesTotal` (total LP solves, 1 or 2 per node), `cutsPerDepth` (histogram of cuts generated by tree depth). Enables diagnosis of cut effectiveness and warm-start degradation without impacting production hot-loop performance. |
