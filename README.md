...
| `v0.7.0` | 🔄 In progress | **B&B log-proof** (intermediate format): machine-verifiable certificate of the B&B search. Activated via `BBOptions::proofStream` (default `nullptr` - disabled). Each node emits: its parent ID (`N`), the single bound-change that created it (`DIFF`), LP bound (`LP`), and its pruning reason - explicit Farkas certificate (`FARKAS_LP` / `FARKAS_BOUND`), bound dominance (`PRUNE_BOUND`), CP infeasibility (`CP_INFEASIBLE`), or an integer-feasible certificate (`INCUMBENT` / `LEAF`). Globally-valid cuts are emitted as `CUT` events. Nodes whose LP solver hit MaxIter or NumericalFailure are logged as `UNVERIFIED`. Output is buffered (64 KB) and thread-safe (lock-free atomic IDs, mutex-serialised writes). |
